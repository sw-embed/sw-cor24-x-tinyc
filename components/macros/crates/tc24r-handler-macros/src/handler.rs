...
///
/// Generates a unit struct and implements `tc24r_dispatch::Handler<Input, State>`
/// with the provided `can_handle` match pattern and `handle` body.
///
/// # Parameters
///
/// - `$name` -- name of the handler struct to create
/// - `$input` -- the input type (e.g., `tc24r_ast::Stmt`)
...
/// # // In real usage, the types come from tc24r-ast and tc24r-codegen-state.
...
macro_rules! define_handler {
    (
        ...
        impl tc24r_dispatch::Handler<$input, $state> for $name {
            fn can_handle(&self, input: &$input) -> bool {
