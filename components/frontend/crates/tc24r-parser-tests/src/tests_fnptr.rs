...
fn parse_fn_ptr_array_decl() {
    ...
    if let Stmt::LocalDecl {
        ty: Type::Array(inner, _),
        ..
    } = &stmts[0]
    {
...
fn parse_fn_ptr_param() {
    ...
    let program =
        parse_source("void apply(int (*f)(int), int x) { return; } int main() { return 0; }");
...
fn parse_indirect_call_postfix() {
    ...
    let program = parse_source("int main() { int (*table[4])(int); table[0](5); return 0; }");
    let stmts = &program.functions[0].body.as_ref().unwrap().stmts;
    // stmt[1] should be Expr(IndirectCall { ... })
    assert!(
        matches!(&stmts[1], Stmt::Expr(Expr::IndirectCall { .. })),
...
fn parse_deref_indirect_call() {
    ...
    let program = parse_source("int main() { int (*fp)(int); (*fp)(3); return 0; }");
    let stmts = &program.functions[0].body.as_ref().unwrap().stmts;
    assert!(
        matches!(&stmts[1], Stmt::Expr(Expr::IndirectCall { .. })),
        "expected IndirectCall, got {:?}",
