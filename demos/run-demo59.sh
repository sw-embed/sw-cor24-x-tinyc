...
DEMO_C="$SCRIPT_DIR/demo59.c"
DEMO_S="$SCRIPT_DIR/demo59.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 59: struct member array element write/read ==="
...
if [ "$R0" = "59" ]; then echo "  [PASS] r0 = 59"; else echo "  [FAIL] r0 = $R0"; PASS=false; fi
if echo "$UART" | grep -q "D59OK"; then echo "  [PASS] UART: D59OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 59 PASSED"; else echo "Demo 59 FAILED"; exit 1; fi
