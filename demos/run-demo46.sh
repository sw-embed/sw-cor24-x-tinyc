...
DEMO_C="$SCRIPT_DIR/demo46.c"
DEMO_S="$SCRIPT_DIR/demo46.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 46: unsigned int, shifts, comparisons ==="
...
if echo "$UART" | grep -q "D46OK"; then echo "  [PASS] UART: D46OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 46 PASSED"; else echo "Demo 46 FAILED"; exit 1; fi
