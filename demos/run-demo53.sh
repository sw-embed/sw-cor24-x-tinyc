...
DEMO_C="$SCRIPT_DIR/demo53.c"
DEMO_S="$SCRIPT_DIR/demo53.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 53: function pointer as parameter ==="
...
if [ "$R0" = "53" ]; then echo "  [PASS] r0 = 53"; else echo "  [FAIL] r0 = $R0"; PASS=false; fi
if echo "$UART" | grep -q "D53OK"; then echo "  [PASS] UART: D53OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 53 PASSED"; else echo "Demo 53 FAILED"; exit 1; fi
