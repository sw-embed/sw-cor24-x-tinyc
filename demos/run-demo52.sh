...
DEMO_C="$SCRIPT_DIR/demo52.c"
DEMO_S="$SCRIPT_DIR/demo52.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 52: function pointer array dispatch ==="
...
if [ "$R0" = "52" ]; then echo "  [PASS] r0 = 52"; else echo "  [FAIL] r0 = $R0"; PASS=false; fi
if echo "$UART" | grep -q "D52OK"; then echo "  [PASS] UART: D52OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 52 PASSED"; else echo "Demo 52 FAILED"; exit 1; fi
