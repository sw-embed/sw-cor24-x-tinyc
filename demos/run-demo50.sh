...
DEMO_C="$SCRIPT_DIR/demo50.c"
DEMO_S="$SCRIPT_DIR/demo50.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 50: large local array (BUG-013 offset overflow) ==="
...
if [ "$R0" = "50" ]; then echo "  [PASS] r0 = 50"; else echo "  [FAIL] r0 = $R0"; PASS=false; fi
if echo "$UART" | grep -q "D50OK"; then echo "  [PASS] UART: D50OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 50 PASSED"; else echo "Demo 50 FAILED"; exit 1; fi
