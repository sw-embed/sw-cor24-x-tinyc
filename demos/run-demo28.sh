...
DEMO_C="$SCRIPT_DIR/demo28.c"
DEMO_S="$SCRIPT_DIR/demo28.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 28: union ==="
...
if echo "$UART" | grep -q "D28OK"; then echo "  [PASS] UART: D28OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 28 PASSED"; else echo "Demo 28 FAILED"; exit 1; fi
