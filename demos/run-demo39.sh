...
DEMO_C="$SCRIPT_DIR/demo39.c"
DEMO_S="$SCRIPT_DIR/demo39.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 39: printf and long branches ==="
...
if echo "$UART" | grep -q "D39OK"; then echo "  [PASS] UART: D39OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 39 PASSED"; else echo "Demo 39 FAILED"; exit 1; fi
