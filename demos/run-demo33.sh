...
DEMO_C="$SCRIPT_DIR/demo33.c"
DEMO_S="$SCRIPT_DIR/demo33.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 33: comma-separated struct members ==="
...
if echo "$UART" | grep -q "D33OK"; then echo "  [PASS] UART: D33OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 33 PASSED"; else echo "Demo 33 FAILED"; exit 1; fi
