...
DEMO_C="$SCRIPT_DIR/demo44.c"
DEMO_S="$SCRIPT_DIR/demo44.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 44: Lisp Phase 1 ==="
...
if echo "$UART" | grep -q "D44OK"; then echo "  [PASS] UART: D44OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 44 PASSED"; else echo "Demo 44 FAILED"; exit 1; fi
