...
DEMO_C="$SCRIPT_DIR/demo36.c"
DEMO_S="$SCRIPT_DIR/demo36.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 36: forward-declared structs ==="
...
if echo "$UART" | grep -q "D36OK"; then echo "  [PASS] UART: D36OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 36 PASSED"; else echo "Demo 36 FAILED"; exit 1; fi
