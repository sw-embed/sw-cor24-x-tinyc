...
DEMO_C="$SCRIPT_DIR/demo60.c"
DEMO_S="$SCRIPT_DIR/demo60.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 60: compound literals ==="
...
if [ "$R0" = "60" ]; then echo "  [PASS] r0 = 60"; else echo "  [FAIL] r0 = $R0"; PASS=false; fi
if echo "$UART" | grep -q "D60"; then echo "  [PASS] UART: D60"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 60 PASSED"; else echo "Demo 60 FAILED"; exit 1; fi
