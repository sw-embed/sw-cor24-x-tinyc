...
DEMO_C="$SCRIPT_DIR/demo26.c"
DEMO_S="$SCRIPT_DIR/demo26.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 26: switch/case ==="
...
if echo "$UART" | grep -q "D26OK"; then echo "  [PASS] UART: D26OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 26 PASSED"; else echo "Demo 26 FAILED"; exit 1; fi
