...
DEMO_C="$SCRIPT_DIR/demo31.c"
DEMO_S="$SCRIPT_DIR/demo31.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 31: tentative definitions ==="
...
if echo "$UART" | grep -q "D31OK"; then echo "  [PASS] UART: D31OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 31 PASSED"; else echo "Demo 31 FAILED"; exit 1; fi
